...
namespace Cong.Class
{
    public class _LOfficeExcel
    {
        ...
        public void xuatKyTen(ref XLWorkbook wb, string[] mKyTen, int dongXuat)
        {
            SqlFunction sqlFun = new SqlFunction(HttpContext.Current.Session.GetConnectionString2());
            DataTable tab = new DataTable();
            tab = sqlFun.GetData(@"SELECT hienNgayInBC,diaDanh,ngayInBaoCao,uyQuyenKT,TLThuTruong,chucDanhKT,nguoiKT,chucDanhTT,thuTruong
            FROM dbo.tblDMDonVi where maDonVipr=N'" + HttpContext.Current.Session.GetDonVi().maDonVi + "'");
            bool hienNgayBC = bool.Parse(tab.Rows[0]["hienNgayInBC"].ToString());
            string ngayIn = tab.Rows[0]["ngayInBaoCao"].ToString();
            string diaDanh = tab.Rows[0]["diaDanh"].ToString();

            if (bool.Parse(tab.Rows[0]["uyQuyenKT"].ToString()) == false) 
            {
                xuatKyTen(ref wb, mKyTen, dongXuat, HttpContext.Current.Session.GetHienNgayInBC(), HttpContext.Current.Session.GetNgayInBC(), HttpContext.Current.Session.GetDiaDanh());
                return;
            }
            var ws = wb.Worksheet(1);
            ws.Cell(mKyTen[0].Split('-')[0].ToString() + dongXuat).Style.Font.SetItalic(true).Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
            ...
            if (hienNgayBC == true)
            {
                ...
                + " tháng " + ngayIn.Substring(3, 2)
                + " năm " + ngayIn.Substring(6, 4);
                ws.Cell(mKyTen[0].Split('-')[0].ToString() + dongXuat).Style.Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center);
            }
            else
            {
                ws.Cell(mKyTen[0].Split('-')[0].ToString() + dongXuat).Value = "......., ngày.....tháng....năm.....";
            ...
            for (int i = 1; i < mKyTen.Length; i++)
            {
                if (mKyTen[i].Split('-')[5].ToString() == "TT")/*Nếu là thủ trưởng*//*Ký thay thủ trưởng*/
                {
                    dongXuat += 1;
                    ws.Row(dongXuat).Style.Font.SetBold(true).Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center).Alignment.SetWrapText(true);
                    ws.Range(mKyTen[i].Split('-')[0].ToString() + dongXuat, mKyTen[i].Split('-')[1].ToString() + dongXuat).Merge();
                    ws.Cell(mKyTen[i].Split('-')[0].ToString() + dongXuat).Value = tab.Rows[0]["TLThuTruong"].ToString();
                    dongXuat += 1;
                    ws.Row(dongXuat).Style.Font.SetBold(true).Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center).Alignment.SetWrapText(true);
                    ws.Range(mKyTen[i].Split('-')[0].ToString() + dongXuat, mKyTen[i].Split('-')[1].ToString() + dongXuat).Merge();
                    ws.Cell(mKyTen[i].Split('-')[0].ToString() + dongXuat).Value = tab.Rows[0]["chucDanhKT"].ToString();
                    dongXuat += 1;
                    ws.Row(dongXuat).Style.Font.SetItalic(true).Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center).Alignment.SetWrapText(true);
                    ws.Range(mKyTen[i].Split('-')[0].ToString() + dongXuat, mKyTen[i].Split('-')[1].ToString() + dongXuat).Merge();
                    ws.Cell(mKyTen[i].Split('-')[0].ToString() + dongXuat).Value = "(Ký, họ tên)";
                    dongXuat += 5;
                    ws.Row(dongXuat).Style.Font.SetBold(true).Alignment.SetHorizontal(XLAlignmentHorizontalValues.Center).Alignment.SetWrapText(true);
                    ws.Range(mKyTen[i].Split('-')[0].ToString() + dongXuat, mKyTen[i].Split('-')[1].ToString() + dongXuat).Merge();
                    ws.Cell(mKyTen[i].Split('-')[0].ToString() + dongXuat).Value = tab.Rows[0]["nguoiKT"].ToString();
